...
assistantScratchPad =
  scratchpadSpawnActionCustom "xterm -name scratchpad -e assistant"
...
myKeys conf@(XConfig {XMonad.modMask = modm}) =
  ...
  , ((modm, xK_d), spawn "exec rofi -show run")
    -- prompt
  , ((modm .|. shiftMask, xK_x), shellPrompt shellPromptConfig)
  ...
  , ( (modm, xK_g)
    , spawn
        "feh --randomize --bg-fill ~/Pictures/wallpapers/generative-stuffs/*")
  ...
  , ( (modm .|. shiftMask, xK_i)
    , spawn "amixer -q sset Master 3%+ && volume-notify")
  , ( (modm .|. shiftMask, xK_d)
    , spawn "amixer -q sset Master 3%- && volume-notify")
...
myLayoutHook = standardLayouts
  where
    standardLayouts =
      ...
      (tiled |||
       reflectTiled ||| Mirror tiled ||| Grid ||| Full ||| simpleTabbed)
    --Layouts
...
myScratchPads =
  ...
  where
    spawnAssistant = myTerminal ++ " --title assistant -e assistant"
...
-- Shell prompt config: source https://wiki.haskell.org/Xmonad/Config_archive/Robert_Manea%27s_xmonad.hs
shellPromptConfig =
  XPC
